...
class ManageProjectsListingPageSettings extends FilamentPage
{
    ...
    {
        $key = Page::KEY_PROJECTS_LISTING_PAGE;

        Page::ensureProjectsListingPageBlock();

        $row = Page::query()->where('key', $key)->first();

        $this->form->fill([
            $key => array_replace_recursive(
                Page::defaultProjectsListingPagePayload(),
    ...
    {
        $defaults = Page::defaultProjectsListingPagePayload();
        ...
        $out = [];
        foreach (is_array($projects) ? $projects : [] as $id) {
            if ($id === null || $id === '') {
                continue;
            }
            $out[] = (int) $id;
        }
        $merged['projects'] = array_values(array_unique($out));
            ...
            ->components([
                Group::make([
                    Section::make('Header')
                        ...
                        ->schema([
                            FileUpload::make('cover_image')
                                ...
                                ->disk('public')
                                ->directory('projects/listing/cover')
                                ->visibility('public')
                                ->image()
                                ->columnSpanFull(),
                    ...
                    Section::make('Projects list')
                        ->description('Optional curated list. If empty, the public page can show all projects.')
                        ->schema([
